...
def add_favorite(bot, update, item: Bot, callback_alert=None):
    ...
    if created:
        text = mdformat.love("{} added to your @BotList {}favorites.".format(fav.bot, '' if callback_alert else '/'))
        if callback_alert:
            update.callback_query.answer(text=text, show_alert=False)
        else:
            msg = util.send_or_edit_md_message(bot, uid, text, to_edit=mid)
            mid = msg.message_id
...
def remove_favorite(bot, update, to_remove):
    return None
...
def add_custom(bot, update, username):
    ...
    try:
        ...
        util.send_or_edit_md_message(bot, uid, mdformat.none_action(
            "{} is already a favorite of yours. /favorites".format(fav.custom_bot)), to_edit=mid)
